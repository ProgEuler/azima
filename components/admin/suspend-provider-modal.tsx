...
const SUSPEND_REASONS = [
	...
	"Menu or pricing misleading",
	"Owner asked us to close it",
	"Other",
] as const;

export interface SuspendProviderModalProps {
	open: boolean;
...
export function SuspendProviderModal({
	open,
	onOpenChange,
	providerName = "Yummy Catering",
	liveOrdersCount = 5,
	onConfirm,
}: SuspendProviderModalProps) {
	const [selectedReason, setSelectedReason] = useState<string>("");
	const [otherReason, setOtherReason] = useState<string>("");

	const ordersSentence =
		liveOrdersCount === 1
			? "1 live order already accepted stays in place"
			: `${liveOrdersCount} live orders already accepted stay in place`;
			...
			>
				<DialogHeader className="gap-2 text-left">
					<DialogTitle className="text-xl sm:text-[22px] font-bold text-[#1C1917] dark:text-stone-100 tracking-tight leading-snug">
						Suspend {providerName}?
					</DialogTitle>
					<DialogDescription className="text-sm sm:text-[14.5px] text-[#57524D] dark:text-stone-300 leading-relaxed pt-0.5">
						{providerName} will disappear from discovery and cannot sign in.{" "}
						{ordersSentence} — cancel them separately if they should not go ahead.
					</DialogDescription>
				</DialogHeader>
